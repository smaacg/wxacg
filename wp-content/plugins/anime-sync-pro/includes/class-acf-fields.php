...
private function register_manga_fields(): void {
    ...
    private function register_faq(): void {
        ...
        // 組 prompt 文字
        // ---------------------------------------------------------------------
        $prompt  = "你是專職編寫「動畫影集」資料的編輯。請上網搜尋我提供的作品，並專門為其『動畫版本』撰寫 FAQ。\n";
        $prompt .= "【極度重要】若該作品為遊戲或漫畫改編，請 100% 圍繞在「動畫版」的劇情與設定！絕對不可提及遊戲操作、遊戲機制（如抽卡、通關）或僅在原作才有的分歧路線。\n\n";
        $prompt .= "【作品名稱】{$title_for_prompt}　\n";
        $prompt .= $extra_line . "\n";
        $prompt .= "搜尋與撰寫規則：\n";
        $prompt .= "1. 動筆前先核對：以上方【補充辨識】的日文原名、年份、媒體形式、季別，確認鎖定的是「正確的作品、正確的季別」且為「動畫版本」。若有同名作品、多季、TV版/劇場版/重製版，務必區分清楚，不可混用不同季別或原作遊戲/漫畫的專屬資訊。\n";
        ...
        $prompt .= "6. 問題須絕對聚焦於「動畫版」的劇情設定、世界觀、角色背景與故事主軸。嚴禁出現與「玩家」、「遊戲操作」、「遊戲機制」相關的任何討論。答案須簡明扼要，不涉及關鍵轉折、結局或重大伏筆（不劇透）。\n";
