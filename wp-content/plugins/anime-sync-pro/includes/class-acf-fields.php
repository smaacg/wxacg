...
/*
 * 漫畫欄位群組已抽到 includes/acf/trait-manga-field-groups.php。
 * 用 trait 而非獨立類別：方法內容原封不動，$this-> 與既有 private helper
 * 照常可用，ACF 的註冊方式與行為完全不變。
 */
require_once __DIR__ . '/acf/trait-manga-field-groups.php';

class Anime_Sync_ACF_Fields {

	use Anime_Sync_ACF_Manga_Field_Groups;
    ...
    /**
     * 全人工短評：組出可直接貼給 AI 的簡短提示詞。
     *
     * ★ 2026-08-17：原本的【已查證資料】欄位、A／B 雙模式、逐項查證來源、
     * EEAT 規範、禁用套語等整套提示詞工程拿掉，改成一句話直接請 AI 寫，
     * 複製貼上後由編輯簡單讀過、抓明顯幻覺或錯誤資訊即可。
     *
    ...
    private function build_editorial_ai_prompt( string $title ): string {
        $title = trim( $title );
        if ( '' === $title ) {
            $title = '（尚未填寫，請先在旁邊「繁中標題」欄位輸入作品名稱）';
        }
        return '給我 ' . $title . ' 的動漫短評';
    }

    /**
     * 動態填入提示詞盒的訊息內容（含繁中標題與單一複製按鈕）。
     *
     * ★ 2026-08-17：拿掉 A／B 雙模式，只剩一顆按鈕、一份極簡提示詞。
     *
    ...
    public function render_editorial_prompt_helper( $field ) {
        ...
        $field['message'] =
            ...
            . '<textarea class="asp-prompt-text" data-mode="a" readonly rows="2" '
	 ...
	 * 是否需要註冊 ACF 欄位群組。
	 *
	 * 前台不需要（模板全走 get_post_meta()）；其餘會用到欄位定義的情境都保留。
	 */
	private static function needs_field_groups(): bool {
		if ( is_admin() ) {
			return true;
		}
	...
	public function register_all_field_groups(): void {
         ...
         * 只在需要編輯介面的情境註冊。
         *
         * 本外掛的前台模板一律用 get_post_meta() 取值，一次都沒用 get_field()，
         * 因此欄位群組定義在前台完全用不到；但 acf/init 每個請求都會跑，
         * 等於每位訪客都要白白建一次這 19 組、數千行的欄位陣列。
         *
         * 保留的情境：後台、AJAX（捷徑盒與 AI 工具都走 admin-ajax）、
         * REST（acf/save_post 與區塊編輯器）、WP-CLI（匯入與回填指令）。
         */
        if ( ! self::needs_field_groups() ) {
            return;
        }
...
$cast_prompt .= "以下是 JSON:\n";
    ...
    'wrapper'       => [ 'width' => '100' ],
                    ...
                    [
                        ...
                        'required'      => 0,
                        // 清單的唯一來源見 class-distributor-registry.php，新增代理商只改那一支
                        'choices'       => Anime_Sync_Distributor_Registry::get_acf_choices(),
                        'default_value' => '',
                        'allow_null'    => 1,
                        'wrapper'       => [ 'width' => '50' ],
                    ],
    ...
    public function inject_shortcut_scripts(): void {
        ...
        jQuery(document).ready(function($) {
            ...
            // 「📋 複製指令」按鈕：ACF message 欄位內容會被 wp_kses 過濾，
        ...
        acf_add_local_field_group( [
            ...
            'fields'                => [
                ...
                [
                    ...
                    'type'          => 'select',
                    // 與真欄位共用同一份清單，見 class-distributor-registry.php
                    'choices'       => Anime_Sync_Distributor_Registry::get_acf_choices(),
                    'default_value' => '',
                    'allow_null'    => 1,
                    'wrapper'       => [ 'width' => '25' ],
