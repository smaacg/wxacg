...
class Anime_Sync_Subject_Relations_Repository {
	 ...
	 *
	 * 分組在數量少時只會製造破洞：一排放得下四張，一組卻只有 1-2 張，
	 * 右邊就空一大截，看起來是好幾排零星小圖而不是一面牆。
	 *
	 * 門檻一開始設 12，實際看正式站才發現不夠：
	 * 《加油！中村同學》19 張分成「片頭曲 1」+「片尾曲 13」+ 其他，
	 * 那個只有一張的組照樣佔掉一整行。分組真正有價值是在「最小的組
	 * 也填得滿一行」的量級，19 張還遠遠不到。
	 *
	 * 25 讓 98% 的作品走單一面牆，只有 26 張以上（實測 16 部）才分組——
	 * 那時最小的組通常也有 4-5 張。
	 *
	 * 正式站專輯數分布：1-6 張 1,012 部 / 7-12 張 170 / 13-25 張 66 /
	 * 26-50 張 12 / 51+ 張 4。
	 *
	...
	/**
	 * 音樂關聯類型。陣列順序就是顯示順序（分組模式是組的順序，
	 * 攤平模式是卡片的順序）。
	 *
	 * 片頭曲／片尾曲排最前面——那是使用者真正在找的東西。
	 *
	 * ★ 這裡曾經是相反的順序，理由是「動畫頁的音樂區塊上半部已經有
	 *   AnimeThemes 的 OP/ED 播放器，3003/3004 是同幾首歌的 CD 單曲，
	 *   擺前面會在同一個區塊裡看到兩次『Blue Bird』」。
	 *
	 *   那個理由在 2026-08-29 改成分頁架構之後就失效了：主題曲播放器
	 *   留在「總覽」，相關專輯在「專輯」分頁，兩者不在同一個畫面，
	 *   重複的問題不存在。留著舊順序只剩壞處。
	 */
	const MUSIC_LABELS = [
		3003 => '片頭曲',
