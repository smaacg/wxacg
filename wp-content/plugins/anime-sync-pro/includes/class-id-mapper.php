...
class Anime_Sync_ID_Mapper {
    ...
    private function resolve_bgm_ext_name_match( string $title, int $season_year, string $season = '' ): ?int {
        ...
        foreach ( $this->build_search_variants( $title ) as $variant ) {
            ...
            /*
             * 輸入帶季號 ≥2、但索引命中的變體沒有季號——那個條目幾乎必定是
             * 第一季，不能拿來當退路。
             *
             * 實例（2026-09-20）：「猫と竜 第2期」被 build_search_variants()
             * 降出變體「猫と竜」，命中 9/14 的舊名稱索引得到 538760（第一季），
             * 於是在這裡被存成 fallback 並回傳，Layer 3 的即時搜尋永遠輪不到。
             * 而即時搜尋其實會把正確的 703539（第2期）排在第 1 筆——它是新建立的
             * 條目，本地索引永遠不會有。同樣成因造成全站 56 組季別塌縮。
             *
             * 847 行的年份保護在這裡幫不上忙：新番檔期未定時 season_year=0，
             * 那個條件整條被跳過。季號是唯一當下就有的判別訊號。
             *
    ...
    private function extract_season_signal( string $title ): array {
        ...
        $patterns = [
            ...
            '/\bcour\s*(\d+)\b/u'                 => 'cour',
            /*
             * クール 系一律歸 'cour'：同一部作品的分割放送，AniList 拆成兩筆、
             * Bangumi 只有一筆涵蓋全部，續作守門必須放行才能正確退回母條目。
             * 不帶「第」的寫法也要收（Dr.STONE SCIENCE FUTURE 3クール、
             * 葬送のフリーレン 2クール）。必須排在下面「第N期/季/部」之前。
             */
            '/第\s*(\d+)\s*クール/u'                => 'cour',
            '/(\d+)\s*クール/u'                     => 'cour',
            /*
             * 以下三條是 2026-09-21 把全站 6,013 筆「現行認不得」的原名整份
             * 盤點後補上的真續作寫法。補之前它們拿到空訊號，而空訊號會讓
             * resolve_bgm_ext_name_match() 命中第一個變體就直接採用、也讓
             * matches_season_signal() 無條件放行——等於完全沒有季號保護。
             *   シーズンN    39 筆（クマーバシーズン3、おでかけ子ザメ シーズン2）
             *   第Nシリーズ  36 筆（キングダム 第6シリーズ、おしりたんてい(第9シリーズ)）
             *   第Nシーズン   8 筆（文豪ストレイドッグス 第2シーズン）
             *
             * ⚠ 同一份盤點確認以下寫法「不可」收進來，那些數字不是季號：
             *   第N話／第N.5話 集數（ガールズ＆パンツァー 最終章 第5話）
             *   第N章          劇場版章節，仍是同一部作品（プリンセス・プリンシパル 第4章）
             *   N回目          作品名的一部分（ループ7回目の悪役令嬢は…）
             *   字尾裸數字     268 筆裡 196 筆的數字是作品名（サンダー３、EZY File 1）
             */
            '/第\s*(\d+)\s*シリーズ/u'              => 'season',
            '/第\s*(\d+)\s*シーズン/u'              => 'season',
            '/シーズン\s*(\d+)/u'                   => 'season',
            /*
             * 「期｜季｜部」前後都不再要求 \b：PCRE 在 /u 下把日文字視為文字
             * 字元，「アニメ4期」的「メ4」之間沒有邊界、「第3期ショート」的
             * 「期シ」之間也沒有，兩側的 \b 都會讓整條 pattern 失效。
             */
            '/第\s*(\d+)\s*(?:期|季|部)/u'          => 'season',
            /*
