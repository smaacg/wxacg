...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	$anime_sync_cli_files = [
		...
		ANIME_SYNC_PRO_DIR . 'includes/class-youranimes-backfill.php',
...
add_action( 'plugins_loaded', function (): void {
	...
	// ------------------------------------------------------
	// Bangumi 主題資料代理（v1.7.7 新增）
	// 前台點專輯開彈窗時取封面／藝術家／發售日，走後端避開 CORS 並快取。
	// ------------------------------------------------------
	if ( class_exists( 'Anime_Sync_Bgm_Subject_Proxy' ) ) {
		Anime_Sync_Bgm_Subject_Proxy::init();
	}
	...
	// ------------------------------------------------------
	// STAFF 完整名單回補（v1.8.3 新增）
	// 拿掉職位白名單之後，既有作品仍只有舊的 6-7 筆，這支把它們補齊。
	// 驅動既有的 ajax_resync_bangumi()，補完自動停。
	// ------------------------------------------------------
	if (
		class_exists( 'Anime_Sync_Staff_Backfill' )
		&& ( is_admin()
			|| ( defined( 'DOING_CRON' ) && DOING_CRON )
			|| ( defined( 'WP_CLI' ) && WP_CLI ) )
	) {
		new Anime_Sync_Staff_Backfill();
	}
