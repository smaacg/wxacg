 ...
 * Changelog:
 *  - 1.4.0 (2026-09-15):
 *      • 新增「📡 串流直接來源」區塊。同日應使用者要求獨立成頁
 *        （admin/pages/streaming-sources.php，選單「📡 串流來源」），
 *        儀表板只留摘要與連結，避免同一份資料兩處維護。
 *  - 1.3.1 (2026-07-20):
...
<div class="wrap asc-dash-wrap">
    ...
    <div class="asc-section">
        <h2 class="asc-section-title">📡 串流直接來源</h2>
        <div class="asc-section-body">
            <?php
            // 只放摘要與連結；完整狀態表在獨立頁 anime-sync-streaming，避免同一份資料兩處維護
            global $wpdb;
            $ss_written = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key LIKE '_anime_tw_streaming_src_%'" );
            $ss_on      = (string) get_option( Anime_Sync_Streaming_Source_Base::WRITE_OPTION, '0' ) === '1';
            ?>
            <p style="margin:0;">
                已接 <?php echo esc_html( count( Anime_Sync_Streaming_Source_Base::available_keys() ) ); ?> 家台灣平台、直接來源已寫入 <strong><?php echo esc_html( number_format_i18n( $ss_written ) ); ?></strong> 部、排程寫入<?php echo $ss_on ? '<span class="asc-status-ok">開啟</span>' : '<span class="asc-status-err">關閉</span>'; ?>。
                <a href="<?php echo esc_url( admin_url( 'admin.php?page=anime-sync-streaming' ) ); ?>" class="button button-small">查看各平台狀態</a>
            </p>
        </div>
    </div>
    <?php endif; ?>
