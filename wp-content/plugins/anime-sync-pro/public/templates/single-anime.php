...
while ( have_posts() ) :
	...
	/*
	 * 尚未播出的作品不可能有已播集數——這裡把它歸零，不信資料庫的值。
	 *
	 * 2026-09-15 事故：《藥師少女的獨語 第三季》（2026-10-02 才開播）頁面顯示
	 * 「已播 24 集」。成因是該篇的 anime_bangumi_id 誤掛成第一季的條目，
	 * AniList 熔斷期間 build_bgm_fallback_media() 取到第一季 24 集的集數表
	 * （2023-10-21 ~ 2024-03-23），合成出 nextAiringEpisode.episode = 25，
	 * 於是寫入 aired = 24。AniList 本身回的是 episode 1，該算成 0。
	 *
	 * 更麻煩的是 Anime_Sync_Meta_Guard 對這個欄位設了 no_decrease，
	 * 錯誤的高值無法被後續同步修正回來，會永久卡住。
	 *
	 * 全站掃描當時有 13 篇處於「狀態未播出、卻有已播集數」的矛盾狀態。
	 * 資料層的防線另外補在 Meta Guard，這裡是顯示層的最後一道：
	 * 就算資料壞了，也不要把不可能的數字端到使用者面前。
	 *
	 * 放在 $is_not_aired 之後、所有使用點之前，$ep_aired 的四個下游
	 * （集數字串、連載中標籤、追番進度條、留言分頁可選集數）一次全對。
	 */
	if ( $is_not_aired ) {
		$ep_aired = 0;
	}
	...
	<div class="asd-wrap">
		...
		<div class="asd-hero-new">
			...
			$hero_facts = [
				...
				[
					/*
					 * 集數排在「每集時長」前面：兩者是同一個問題的兩半
					 *（總共幾集 × 一集多長），連著看才有意義。
					 * 原本放在側欄，2026-09-01 依使用者要求搬進資料條——
					 * 側欄那份同時移除，不要讓集數又變成出現兩次
					 *（見本區塊開頭關於「集數曾經出現三次」的說明）。
					 */
					'key' => '集數',
					'val' => $ep_str,
				],
			...
			>
				...
				$hero_side_facts = [
					...
					/* 集數已移到 Hero 資料條（2026-09-01），這裡不再重複 */
				];
