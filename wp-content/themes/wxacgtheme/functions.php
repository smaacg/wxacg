...
function wxacg_wpdiscuz_spoiler_button(): void {
	if (
		! is_singular(
			[ 'anime', 'manga', 'asa_char_comments' ]
		)
	) {
		return;
	}
	...
	(function () {
		...
		function insertSpoiler(toolbar) {
			...
			var editorId = toolbarId.replace('wpd-editor-toolbar-', '');
			var editor = document.getElementById('wpd-editor-' + editorId);

			if (!editor) {
				return;
			}

			var content = editor.querySelector('.ql-editor');
			...
			title = escapeShortcodeAttribute(
				title.trim() || '暴雷'
			);
		...
		function addButton(toolbar) {
			if (
				!toolbar ||
				toolbar.querySelector('.wpd-smacg-spoiler-btn')
			) {
				return;
			}
			...
			button.type = 'button';
			button.className = 'ql-smacg-spoiler wpd-smacg-spoiler-btn';
			button.title = '暴雷隱藏';
			button.setAttribute('aria-label', '插入暴雷標記');
		...
		function scan(root) {
			var scope = root || document;
			var toolbars = scope.querySelectorAll
				? scope.querySelectorAll('[id^="wpd-editor-toolbar-"]')
				: [];

			toolbars.forEach(addButton);
		}
		...
		var observer = new MutationObserver(function (mutations) {
			mutations.forEach(function (mutation) {
				mutation.addedNodes.forEach(function (node) {
					...
					if (
						node.id &&
						node.id.indexOf('wpd-editor-toolbar-') === 0
					) {
						addButton(node);
					} else {
						scan(node);
...
function wxacg_wpdiscuz_comment_author(
	$author_name,
	$comment
) {
	$user_id = isset( $comment->user_id )
		? (int) $comment->user_id
		: 0;
